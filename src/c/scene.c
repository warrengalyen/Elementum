...
void dirt_scene(void) {
    ...
    while (i-- > 0) {
        ...
        if (target->y >= height - 100 && randEveryU8(4))
            spawnElement(target, target->y >= height - 50 ? COBBLESTONE : DIRT);
    }

    Cell *seedCell = getCell(randomU32() % width, 0);
    if (getType(seedCell) == EMPTY)
        spawnElement(seedCell, SEED);
};
...
void snow_scene(void) {
    ...
    while (i-- > 0) {
        ...
        if (target->y >= height - 100 && randEveryU8(4))
            spawnElement(target, target->y >= height - 50 ? DIRT : SNOW);
...
void sand_scene(void) {
    ...
    while (i-- > 0) {
        ...
        if (target->y >= height - 100 && randEveryU8(4)) {
            if ((!right && target->x >= width / 3) || (right && target->x <= width - width / 3))
                spawnElement(target, SAND);
            else
