...
void getMooreNeighborhood(Cell *c, Cell **out) {
    ...
    if (y > 0) {
        if (x)
            out[0] = &cells[(i - width) - 1]; // top left
        out[1] = &cells[i - width];           // top
        if (x < width - 1)
            out[2] = &cells[(i - width) + 1]; // top right
    }

    if (x > 0)
        out[3] = &cells[i - 1]; // left
    if (x < width - 1)
        out[4] = &cells[i + 1]; // right

    if (y < height - 1) {
        if (x > 0)
            out[5] = &cells[(i + width) - 1]; // bottom left
        out[6] = &cells[i + width];           // bottom
        if (x < width - 1)
            out[7] = &cells[i + width + 1]; // bottom right
    }
}
...
void moveHorizontally(Cell *source, I16 distance, _Bool (*callback)(Element *, Cell *, Cell *)) {
    ...
    if (distance < 0 && x < -distance)
        dx = 0;
    else if (distance > 0 && width - distance <= width)
        dx = width - 1;
    else
        dx = x + distance;
    ...
    while (x != dx) {
        ...
        x += dir;
        target = &cells[ind];
        if (!callback(source->el, source, target))
            break;
        source = target;
    }
}
...
void moveVertically(Cell *source, I16 distance, _Bool (*callback)(Element *, Cell *, Cell *)) {
    ...
    if (distance < 0 && y < -distance)
        dy = 0;
    else if (distance > 0 && height - y <= distance)
        dy = height - 1;
    else
        dy = y + distance;
...
void lineMove(U16 x0, U16 y0, I16 x1, I16 y1, _Bool (*callback)(Element *, Cell *, Cell *)) {
    if (!callback || x0 < 0 || x0 >= width || y0 < 0 || y0 >= height)
        return;
    if (x1 < 0)
        x1 = 0;
    else if (x1 >= width)
    ...
    for (;;) {
        ...
        if (!callback(source->el, source, next))
            break;
        source = next;
        if (x0 == x1 && y0 == y1)
            break;
...
void explosionRay(U16 x0, U16 y0, I16 x1, I16 y1, U8 power) {
    ...
    for (;;) {
        ...
        if (curPower) {
            ...
            if (source->el && elementLookup[source->el->type].state == s_SOLID)
                source->el->scorched = 1;
            if (randEveryU8(10))
                break;
        }
...
inline void attemptHalt(Element *el) {
    ...
    if (el->sbpy >= info->weight - 1)
        el->sbpx = el->sbpy * LORDIR * info->solid.impactScatter;
    else
        el->halted = 1;
}
...
void tryReleaseHalted(Cell *c) {
    ...
    for (U8 i = 0; i < 8; ++i) {
        ...
        if (check && check->el && !elementLookup[check->el->type].isStatic && !check->el->halted &&
            randEveryU8(info->solid.inertia)) {
            c->el->halted = 0;
            return;
        }
    }
...
#undef ADDENTRY
    ...
    for (U16 i = 0; i < type_length; ++i) {
        if (!elementLookup[i].attempt)
            elementLookup[i].attempt = default_attempt;
        HSL base = elementLookup[i].baseColor;
        if (base.lig) {
            for (U8 j = 0; j < 8; ++j) {
                elementLookup[i].colors[j] = hslToRgb(
                    (HSL){.hue = i == PELLET || i == FIREWORK ? randomF32() : base.hue + (randomF32() * 0.05 - 0.025),
                          .sat = base.sat,
