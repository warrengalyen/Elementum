...
void createSubatomic(U16 x, U16 y, U8 type) {
    ...
    if (type == 0) {
        for (U8 i = 0; i < 7; ++i)
            createSubatomicHelper(x, y, i, dir);
    } else
        createSubatomicHelper(x, y, type, dir);
}
...
void tickSubatomics(void) {
    for (Subatomic *node = rootSA, *prev = NULL; node; prev = node, node = node->next) {
        ...
        if (node->waveLength < 7) {
            ...
            case CLONER:
                if (randEveryU8(255))
                    cell->el->r0 = randomU8() % type_length;
            case UNBREAKABLECLONER:
                if (cell->el->r0 == 0)
                    cell->el->r0 = ELECTRON;
            case ELECTRON:
...
#define COLLISIONLENGTH 7
            ...
            default:
                ...
                if (cell->el->electricityState)
                    cell->el->electricityState = 0;
                break;
            case NEUTRON:
                freeCell(cell);
                spawnElement(cell, randEveryU8(3)
                                       ? neutronCollision[randomU8() % COLLISIONLENGTH]
                                       : neutronCollision[(U8)MAX(
                                             0, MIN(COLLISIONLENGTH, cell->temperature / 1000.0f * COLLISIONLENGTH))]);
            case VOID:
                goto DELETE_PARTICLE;
            case PROTON:
            case ELECTRON:
