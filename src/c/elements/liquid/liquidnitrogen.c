...
void VARPREF(update)(Element *el, Cell *cell, U16 x, U16 y) {
    ...
    if (randEveryU8(4)) {
        ...
        if (getType(target) == EMPTY)
            spawnElement(target, FOG);
    }

    if (!attempt_liquidnitrogen(el, cell, getCell(x, y + 1))) {
        if (cell->el && cell->el->type == LIQUIDNITROGEN)
            moveHorizontally(cell, (randomU8() % self->liquid.dispersion + 1) * LORDIR, self->attempt);
    };
}
...
ElementInfo VARPOST(info) = {
                             ...
                             .state = s_LIQUID,
                             .liquid = (LiquidInfo){.dispersion = 4, .density = 4},
