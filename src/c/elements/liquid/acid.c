...
void VARPREF(update)(Element *el, Cell *cell, U16 x, U16 y) {
    if (!attempt_acid(el, cell, getCell(x, y + 1))) {
        if (cell->el->type == ACID)
            moveHorizontally(cell, (randomU8() % elementLookup[ACID].liquid.dispersion + 1) * LORDIR, attempt_acid);
    };
}
...
ElementInfo VARPOST(info) = {
                             ...
                             .state = s_LIQUID,
                             .liquid = (LiquidInfo){.dispersion = 3, .density = 0xff},
